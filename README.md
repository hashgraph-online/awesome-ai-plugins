...
- [Notion MCP Server](https://github.com/awkoy/notion-mcp-server) - Token-authenticated Notion access for AI agents through `notion_read` and `notion_write`, covering pages, databases, blocks, comments and file uploads with markdown in and out, batched multi-item calls, an opt-in confirmation prompt before destructive operations, and per-operation allow/block lists.
