...
- [billion-context](https://github.com/ranxianglei/billion-context) - Universal context-compression proxy for AI coding agents — any agent that can set a base URL gets model-driven, incremental, reversible, prefix-cache-friendly compression with zero per-agent adapter code.
- [billion-context-pi](https://github.com/ranxianglei/billion-context-pi) - Billion-Context for the Pi runtime — the model chooses when and what to compress into labeled, multi-tier, searchable blocks (never touching protected or recent messages), keeping a single session lean under ~150K tokens.
...
- [opencode-acp](https://github.com/ranxianglei/opencode-acp) - OpenCode plugin where the model decides when and what to compress — reversible compress/decompress tools with three-tier summaries keep a single session under ~200K tokens instead of silently truncating at a hard limit.
