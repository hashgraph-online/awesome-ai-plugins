...
- [Awesome-Journal-Skills](https://github.com/brycewang-stanford/Awesome-Journal-Skills) - Claude Code plugin marketplace (also usable as Codex skills) of 300 journal- and conference-specific packs covering topic fit, house style, submission rules, and reviewer response for 744 venues across economics, social science, natural science, medicine, and computer science.
...
- [gm-apprentice](https://github.com/AntTheLimey/gm-apprentice) - Claude Code plugin of nine tabletop RPG Game Master skills for campaign creation, session prep, at-the-table support, post-session wrap-up and vault audits that work with any game system, with deep rules support for Call of Cthulhu 7e, GURPS 4e, Forged in the Dark, D&D 5e and Pathfinder 2e and an option to publish the campaign as a website for players.
...
- [Munim](https://github.com/vishalsg42/munim) - Multi-account MCP server that holds a separate OAuth session per client across 11 providers, forwards each provider's own tools with that client's credentials, and refuses any call that would send one client's credential to another host.
...
- [Neo](https://github.com/Parslee-ai/neo) - Code reasoning plugin for Claude Code and Codex that runs multi-agent analysis over relevant repository files and keeps a local semantic memory, promoting a lesson only after its suggestions are verified as accepted in git.
...
- [omp-rewrite](https://github.com/zPeppOz/omp-rewrite) - Oh My Pi (omp) extension that adds a /rewrite command, which asks clarifying questions about a rough draft prompt in a side turn and puts a thorough rewrite in the composer.
...
- [PageSpeed Optimizer](https://github.com/alexlivre/pagespeed-optimizer-alexlivre) - Optimizes web applications for 100/100 PageSpeed Insights, Core Web Vitals, and Generative Engine Optimization (GEO).
...
- [Promptiff](https://github.com/BrantonLiu/promptiff) - Compares AI responses against original user prompts to surface possible omissions and unsourced additions; local CLI and Skill with a comparison canvas.
...
- [RowTrail](https://github.com/adam2go/rowtrail) - Local CLI and MCP server for agents to explore CSV/Parquet with bounded responses, versioned results, and reusable analysis handoffs.
...
- [telepathy](https://github.com/Winterrks/telepathy) - Lets coding-agent sessions on one machine (Claude Code, Codex, OpenCode, Gemini CLI, Copilot CLI, Cursor, Grok, Devin, Antigravity, Kimi Code, Qwen Code, Kilo Code) find and message each other through the same three MCP tools, starting a turn in an idle session where the agent allows it, with no network calls or telemetry.
...
- [claude-fuer-deutsches-recht](https://github.com/Klotzkette/claude-fuer-deutsches-recht) - Experimental German-law skill collection with installable plugins, drafting workflows, legal source checks, and practice case files.
...
- [Jev Social](https://github.com/socai-io/jev-social) - Agent skill that runs read-only Instagram, TikTok, and LinkedIn research via browser automation and returns source-linked evidence reports.
...
- [mcp-music-studio](https://github.com/linxule/mcp-music-studio) - MCP server for AI music creation: scored ABC notation composition and Strudel live coding, with sheet music rendering, harmony analysis, and MIDI/WAV export.
...
- [opencode-agy-auth](https://github.com/anthonyhaussman/opencode-agy-auth) - OpenCode plugin providing OAuth 2.0 authentication, model fetching, and quota tracking for the Antigravity CLI.
- [opencode-cache-hit](https://github.com/zhumengzhu/opencode-cache-hit) - OpenCode sidebar plugin showing prompt cache hit rate, token usage, and cost with sub-agent session rollup.
...
- [SpringBrand](https://github.com/springbrand-lab/springbrand-agent-setup) - Hosted go-to-market MCP server with setup guides for Claude Code, Codex, Cursor and other agents: social listening across X, TikTok, Instagram, YouTube, Reddit and Xiaohongshu, website traffic and SEO research, company, contact and creator discovery, and copy, image, video and voiceover generation over OAuth, billed per call.
