...
- [dsh-plugin-kit](https://github.com/hyzyn/dsh-plugin-kit) - Plugin collection for the DeepSeek Harness Web GUI (`@hyzyn/dsh-plugin-kit`): MCP server config, SSH/SFTP terminal panel, Docker container panel, RSS digest, global search, profile / prompt / secret management and Codegraph integration; install with `dsh plugin --profile web add @hyzyn/dsh-plugin-kit`.
