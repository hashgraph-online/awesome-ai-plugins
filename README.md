...
- [dsh-plugin-shop](https://github.com/LivXue/dsh-plugin-shop) - Plugin shop for DeepSeek Harness: discover, install, enable and update dsh plugins from a browsable, git-auditable catalog rebuilt daily from npm keywords and GitHub topics, with every filtered-out package published alongside the reason it was excluded; install with `dsh plugin --profile web add dsh-plugin-shop`.
