...
- [QVeris Agent Toolkit](https://github.com/QVerisAI/qveris-agent-toolkit) - Cross-client MCP toolkit for discovering, inspecting, quoting, and calling external tools with usage and settlement audits.
...
- [dsh-deja](https://github.com/vshulcz/deja-vu)
- [dsh-memory](https://github.com/FuRongJun-1999/dsh-memory) - Lingshu long-term memory for AI agents that persists conversations as a rule-governed markdown cognitive graph served as a stdio MCP server — deterministic gates decide what is stored and retrieved (no LLM black box), a zero-dependency Rust core powers retrieval (99.0% hit@1, top of a public six-system Chinese benchmark), and the same brain mounts into DSH, CodeBuddy, ZCode, Codex CLI, and Claude Code; install with `dsh plugin add` (npm `@furongjun1999/dsh-memory`). - Brings the session history of nineteen other coding agents into DeepSeek Harness: recall, session digest and per-file history tools over a local index, plus optional automatic recall.
